...
import com.vkzwbim.chat.db.dao.MsgRoamTaskDao;
...
import com.vkzwbim.chat.util.DateFormatUtil;
...
public class XMuChatMessageListener implements MessageListener {
    ...
    public void processMessage(Message message) {
        ...
        if (com.vkzwbim.chat.util.StringUtils.strEquals(message.getPacketID(), "") || message.getPacketID() == null) {
            /**
             ...
             * @see {@link  com.vkzwbim.chat.bean.message.NewFriendMessage#toJsonString(boolean)}
             * */
            try {
    ...
    private void saveGroupMessage(String body, String from, String packetId, boolean isDelay) {
        ...
        if (type == XmppMessage.TYPE_83) {
            ...
            LogUtil.e("  红包领取方  "+fromUserId+"    红包发送方  "+toUserId);
            String fromName="";
            String toName="";
            if (TextUtils.equals(fromUserId, mLoginUserId) && TextUtils.equals(toUserId, mLoginUserId)) {// 自己领取了自己的红包
                ...
                fromName = MyApplication.getContext().getString(R.string.you);
                toName = RoomMemberDao.getInstance().getRoomMemberName(chatMessage.getObjectId(), chatMessage.getToUserId());
            }

            else {// xx领取了xx的红包

                fromName = chatMessage.getFromUserName();
                toName = RoomMemberDao.getInstance().getRoomMemberName(chatMessage.getObjectId(), toUserId);
            }
          ...
          }else if (TextUtils.equals(fromUserId, mLoginUserId)){
              chatMessage.setFileSize(XmppMessage.TYPE_83);
              chatMessage.setFilePath(chatMessage.getContent());
              chatMessage.setType(XmppMessage.TYPE_TIP);
              chatMessage.setContent(str);
        ...
        if (type == XmppMessage.TYPE_BACK) {
            ...
            intent.setAction(com.vkzwbim.chat.broadcast.OtherBroadcast.MSG_BACK);
    ...
    private void chatGroup(String body, ChatMessage chatMessage, Friend friend) {
        ...
        if (type == XmppMessage.TYPE_CHANGE_NICK_NAME) { // 修改群内昵称
            ...
            if (ChatMessageDao.getInstance().saveNewSingleChatMessage(mLoginUserId, friend.getUserId(), chatMessage)) {
                ...
                intent.setAction(com.vkzwbim.chat.broadcast.OtherBroadcast.REFRESH_MANAGER);
                mService.sendBroadcast(intent);
            }
        }
    }
