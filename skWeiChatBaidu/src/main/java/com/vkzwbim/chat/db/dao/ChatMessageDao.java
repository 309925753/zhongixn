package com.vkzwbim.chat.db.dao;
...
import com.vkzwbim.chat.db.SQLiteHelper;
import com.vkzwbim.chat.db.SQLiteRawUtil;
import com.vkzwbim.chat.db.UnlimitDaoManager;
...
public class ChatMessageDao {
    ...
    public boolean saveNewSingleChatMessage(String ownerId, String friendId, ChatMessage message) {
        ...
        Log.e("ormlite", "开始存消息"+message.getContent());
     ...
     * @param state
     */
    public void updateMessageDice(String ownerId, String friendId, String packetId, boolean state) {
        LogUtil.e("cjh 更ixn 骰子 "+ownerId+"  friendId "+friendId+  "  packetId "+packetId);
        Dao<ChatMessage, Integer> dao = getDao(ownerId, friendId);
        try {
        if (dao == null) {
            Log.e("xuan", "更新骰子显示状态失败:" + packetId);
            return;
        }

        UpdateBuilder<ChatMessage, Integer> builder = dao.updateBuilder();

            builder.updateColumnValue("isDiceSelect", 1);
    ...
    public void updateMessageRead(String ownerId, String friendId, String packetId, boolean state) {
        LogUtil.e("cjh 更ixn "+ownerId+"  friendId "+friendId+  "  packetId "+packetId);
        Dao<ChatMessage, Integer> dao = getDao(ownerId, friendId);
        if (dao == null) {
     ...
     * 红包、转账的领取状态
     * */
    public boolean updateChatMessageReceiptStatus(String ownerId, String friendId, String packetId, String statue) {
        try {
            Dao<ChatMessage, Integer> dao = getDao(ownerId, friendId);
            UpdateBuilder<ChatMessage, Integer> builder = dao.updateBuilder();
            ChatMessage message = findMsgById(ownerId, friendId, packetId);
            if (message != null) {
                LogUtil.e("cjh red555 "+statue);

                builder.updateColumnValue("fileSize", statue);
                builder.where().eq("packetId", packetId);
                dao.update(builder.prepare());
            }
            return true;
    ...
    public void updateMessageContentTimeLen(String ownerId, String friendId, String packetId, String timelen) {
        Dao<ChatMessage, Integer> dao = getDao(ownerId, friendId);
        if (dao == null) {
            return;
        }
        UpdateBuilder<ChatMessage, Integer> builder = dao.updateBuilder();
        try {
            builder.updateColumnValue("videoLenth", timelen);
            builder.where().eq("packetId", packetId)            ;
            dao.update(builder.prepare());
    ...
    public void updateMessageState(String ownerId, String friendId, String packetId, int messageState) {
        ...
        try {
            ...
            builder.where().eq("packetId", packetId);
            dao.update(builder.prepare());
            LogUtil.e("cjh消息更新  消息发送状态更新成功-->packetId：" + packetId + "，messageState" + messageState);
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    //
    public ChatMessage getDiceChatMessage(String ownerId, String friendId,String packetId) {
        Dao<ChatMessage, Integer> dao = getDao(ownerId, friendId);
        ...
        try {
            QueryBuilder<ChatMessage, Integer> builder = dao.queryBuilder();
            builder.where().ne("type", XmppMessage.TYPE_GIF)
