...
class RedViewHolder extends AChatHolderInterface {
    ...
    public void initView(View view) {
        ...
        tv_red_statue = view.findViewById(R.id.tv_red_statue);
    ...
    public void fillData(ChatMessage message) {
        ...
        if (mdata.getFileSize() == 2) {// 已领取
            if (mdata.isMySend()) {
                mRootView.setBackgroundResource(R.mipmap.right_red);
            } else {
                mRootView.setBackgroundResource(R.mipmap.left_red);
            }
            iv_image.setImageResource(R.drawable.redpackopen);
            tv_red_statue.setText("已领取");

        } else if (mdata.getFileSize() == 1121) {
            if (mdata.isMySend()) {
                mRootView.setBackgroundResource(R.mipmap.right_red);
            } else {
                mRootView.setBackgroundResource(R.mipmap.left_red);
            ...
            iv_image.setImageResource(R.drawable.redpackopen);
            tv_red_statue.setText("已领完");
        } else {
            if (mdata.isMySend()) {
                mRootView.setBackgroundResource(R.mipmap.right_red_open);
            } else {

                mRootView.setBackgroundResource(R.mipmap.left_red_open);
            }
            iv_image.setImageResource(R.drawable.yy_red_pic);
            tv_red_statue.setText("");
        }
                ...
                .execute(new BaseCallback<OpenRedpacket>(OpenRedpacket.class) {
                    ...
                    public void onResponse(ObjectResult<OpenRedpacket> result) {
                        if (result.getData() != null) {
                            ...
                            if (openRedpacket.getPacket().getStatus() == 2) {
                                mdata.setFileSize(1121);
                                ChatMessageDao.getInstance().updateChatMessageReceiptStatus(mLoginUserId, mToUserId, mdata.getPacketId(),"1121");
                                fillData(mdata);
                            }
                        ...
                        if (result.getData() != null) {
                            if(openRedpacket.getPacket().getStatus() == 2){
                                ...
                                ChatMessageDao.getInstance().updateChatMessageReceiptStatus(mLoginUserId, mToUserId, mdata.getPacketId(),"1121");
                                 ...
                                 mdata.setFileSize(2);
                            ChatMessageDao.getInstance().updateChatMessageReceiptStatus(mLoginUserId, mToUserId, mdata.getPacketId(),"2");
                            }
                            fillData(mdata);
                            ...
                            if (!TextUtils.equals(mLoginUserId, openRedpacket.getPacket().getUserId()) && !isGounp) {
