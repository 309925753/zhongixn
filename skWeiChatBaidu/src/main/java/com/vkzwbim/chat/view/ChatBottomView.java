...
import com.vkzwbim.chat.audio.IMRecordController;
import com.vkzwbim.chat.audio.RecordListener;
...
import com.vkzwbim.chat.util.InputManager;
...
import com.vkzwbim.chat.util.filter.EmojiInputFilter;
import com.vkzwbim.chat.util.input.KeyboardPanelUtil;
...
public class ChatBottomView extends LinearLayout implements View.OnClickListener {
    private static final int LIMIT_MESSAGE_LENGTH = 10000;
    ...
    private void init(Context context) {
        ...
        mChatEdit.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable editable) {
                if (editable.length() > LIMIT_MESSAGE_LENGTH) {
                    toastTextLimit();
                    editable.delete(LIMIT_MESSAGE_LENGTH, editable.length());
                }
            }
        });
    ...
    private void changeChatFaceView(boolean show) {
        ...
        if (show) {
            ...
            android.support.v4.view.ViewCompat.setBackgroundTintList(mEmotionBtn, ColorStateList.valueOf(getResources().getColor(R.color.dan_gu_lv)));
        } else {
            mChatFaceView.setVisibility(View.GONE);
            android.support.v4.view.ViewCompat.setBackgroundTintList(mEmotionBtn, null);
        }
    }
