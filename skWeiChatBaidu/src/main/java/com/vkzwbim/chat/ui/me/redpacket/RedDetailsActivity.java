...
public class RedDetailsActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void initView() {
        ...
        findViewById(R.id.ll_back_left).setOnClickListener(this);
    ...
    private void showData() {
        ...
        if (packetEntity.getUserId().equals(coreManager.getSelf().getUserId())){

            if (list.size() == packetEntity.getCount()){
                resultMsg =packetEntity.getCount()+"个红包，共"+ df.format(packetEntity.getMoney())+"元";
            }else{
            ...
            if (list.size() == packetEntity.getCount()){
                resultMsg = packetEntity.getCount()+"个红包";

            }else{

            resultMsg = "已领取"+list.size()+"/"+packetEntity.getCount()+"个";
            }
        }
        ...
        if (list.size() == packetEntity.getCount()) {
          ...
          for (OpenRedpacket.ListEntity d: list){
              LogUtils.e("cjh ","cjh时间   "+d.getUserName()+"  时间 "+d.getTime() );
            }
            ...
            long redSendTime = packetEntity.getSendTime();
            long betweenTime = list.get(0).getTime() - redSendTime;
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.iv_title_left || v.getId() == R.id.ll_back_left) {
            finish();
