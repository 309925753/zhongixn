...
import com.vkzwbim.chat.view.SelectCpyPopupWindow;
        ...
        public StructHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            ...
            StructHolder holder = new StructHolder(view, new ItemClickListener() {
                ...
                public void onItemSelectChange(int layoutPosition, boolean selected) {
                    ...
                    int realIndex = -1;
                    for (int i = 0; i < mData.size(); i++) {
                        StructBean next = mData.get(i);
                        if (realIndex == -1) {
                            if (TextUtils.equals(next.getId(), bean.getId())) {
                                realIndex = i;
                            }
                            continue;
                        }
                        // 选中公司或者部门就全选，
                        if (type(next) <= type(bean)) {
                            break;
                        }
                        next.setSelected(selected);
                    }
                    boolean allSelected = selected;
                    for (int i = realIndex + 1; i < mData.size(); i++) {
                        StructBean next = mData.get(i);
                        // 选中员工或者部门要判断所在部门或者公司是否全选，
                        if (next.isCompany()) {
                            break;
                        }
                        if (type(next) >= type(bean)) {
                    ...
                    if (!bean.isCompany()) {
                        for (int i = realIndex - 1; i >= 0; i--) {
                            ...
                            } else if (TextUtils.equals(bean.getParent_id(), next.getId())) {
                                next.setSelected(allSelected);
                            }
                            if (next.isCompany()) {
                                next.setSelected(allSelected);
                                break;
